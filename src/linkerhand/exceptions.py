"""Custom exceptions for the Linkerhand SDK.
...
class LinkerHandError(Exception):
    """Base exception for all Linkerhand SDK errors.
...
class TimeoutError(LinkerHandError):
...
class CANError(LinkerHandError):
...
class ValidationError(LinkerHandError):
...
class StateError(LinkerHandError):

"""Torque control and sensing for O7 robotic hand.
...
@dataclass
class O7Torque:
    """Joint torques for O7 hand (0-100 range or milliamps).
    ...
    Attributes:
        ...
        thumb_rotation: Thumb rotation joint torque (0-100). Higher values = higher torque limit.
    """
    ...
    def to_list(self) -> list[float]:
        ...
        Returns:
            List of 7 joint torques [thumb_flex, thumb_abd, index, middle, ring, pinky, thumb_rotation]
        """
        return [
            self.thumb_flex,
            self.thumb_abd,
    ...
    @classmethod
    def from_list(cls, values: list[float]) -> "O7Torque":
    ...
    @classmethod
    def from_raw(cls, values: list[int]) -> "O7Torque":
    ...
    def to_milliamps(self) -> list[float]:
        ...
        Returns:
            List of 7 joint torques in mA [thumb_flex, thumb_abd, index, middle, ring, pinky, thumb_rotation]

        Example:
            >>> torque = O7Torque(50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0)
    ...
    @classmethod
    def from_milliamps(cls, ma_values: list[float]) -> "O7Torque":
        """Construct from list of torques in milliamp units.

        Args:
            ma_values: List of 7 torque values in milliamps (0 to 1657.5 mA)
        ...
        Example:
            >>> torque = O7Torque.from_milliamps([800.0, 800.0, 800.0, 800.0, 800.0, 800.0, 800.0])
        """
        if len(ma_values) != 7:
            raise ValueError(f"Expected 7 values, got {len(ma_values)}")
    ...
    def __len__(self) -> int:
        """Return number of joints (always 7 for O7)."""
        return 7
...
class TorqueData:
    ...
    Attributes:
        torques: O7Torque instance containing joint torques (0-100 range).
...
class TorqueManager:
    ...
    1. Torque control: set_torques() - send 7 target torques and cache response
    2. Blocking mode: get_blocking() - request and wait for 7 current torques
    ...
    def set_torques(self, torques: O7Torque | list[float]) -> None:
        """Send target torques to the robotic hand.

        Args:
            torques: O7Torque instance or list of 7 target torques (range 0-100 each).

        Raises:
            ValidationError: If torques count is not 7 or values are out of range.

        Example:
            >>> manager.set_torques(O7Torque(thumb_flex=50.0, thumb_abd=30.0,
            ...                              index=60.0, middle=60.0, ring=60.0,
            ...                              pinky=60.0, thumb_rotation=50.0))
            >>> manager.set_torques([50.0, 30.0, 60.0, 60.0, 60.0, 60.0, 50.0])
        """
        if isinstance(torques, O7Torque):
            raw_torques = torques.to_raw()
        elif isinstance(torques, list):
            raw_torques = O7Torque.from_list(torques).to_raw()
        else:
            raise ValidationError(
                f"Expected O7Torque or list, got {type(torques).__name__}"
            )
    ...
    def _on_message(self, msg: can.Message) -> None:
        ...
        torques = O7Torque.from_raw(raw_torques)

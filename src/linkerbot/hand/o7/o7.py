"""O7 robotic hand control interface.

This module provides the main O7 class for controlling the O7 robotic hand
...
class O7:
    """Main interface for O7 robotic hand control.

    This class provides a unified interface for controlling the O7 robotic hand,
    integrating angle control, speed control, sensor data acquisition, and
    unified sensor streaming.

    The O7 class should be used as a context manager to ensure proper resource
    cleanup:
    ...
    def __init__(
        ...
        """Initialize the O7 robotic hand interface.
    ...
    def get_snapshot(self) -> O7Snapshot:
        """Get all sensor data as a single snapshot (non-blocking).

        Returns:
            O7Snapshot with the latest cached data from all sensors.
    ...
    def start_polling(
        ...
        for source in sources:
            t = threading.Thread(
                ...
                args=(source.value,),
                daemon=True,
                name=f"O7-Polling-{source.value}",
            )
            t.start()
    ...
    def close(self) -> None:
        """Close the O7 interface and release all resources.
    ...
    def _ensure_open(self) -> None:
        ...
        if self._closed:
            raise StateError(
                "O7 interface is closed. Create a new instance or use context manager."
            )

"""Angle control and sensing for O7 robotic hand.
...
@dataclass
class O7Angle:
    """Joint angles for O7 hand (0-100 range).
    ...
    Attributes:
        ...
        thumb_rotation: Thumb rotation joint angle (0-100). Higher values rotate the thumb.
    """
    ...
    def to_list(self) -> list[float]:
        ...
        Returns:
            List of 7 joint angles [thumb_flex, thumb_abd, index, middle, ring, pinky, thumb_rotation]
        """
        return [
            self.thumb_flex,
            self.thumb_abd,
    ...
    @classmethod
    def from_list(cls, values: list[float]) -> "O7Angle":
    ...
    @classmethod
    def from_raw(cls, values: list[int]) -> "O7Angle":
...
class AngleData:
    ...
    Attributes:
        angles: O7Angle instance containing joint angles (0-100 range).
...
class AngleManager:
    ...
    1. Angle control: set_angles() - send 7 target angles and cache response
    2. Blocking mode: get_blocking() - request and wait for 7 current angles
    ...
    def set_angles(self, angles: O7Angle | list[float]) -> None:
        """Send target angles to the robotic hand.

        This method sends 7 target angles to the hand. The hand will respond
        with the current angles, which are automatically cached and can be
        retrieved via get_snapshot().

        Args:
            angles: O7Angle instance or list of 7 target angles (range 0-100 each).

        Raises:
            ValidationError: If angles count is not 7 or values are out of range.

        Example:
            >>> manager = AngleManager(arbitration_id, dispatcher)
            >>> manager.set_angles(O7Angle(thumb_flex=50.0, thumb_abd=30.0,
            ...                            index=60.0, middle=60.0, ring=60.0,
            ...                            pinky=60.0, thumb_rotation=50.0))
            >>> manager.set_angles([50.0, 30.0, 60.0, 60.0, 60.0, 60.0, 50.0])
        """
        if isinstance(angles, O7Angle):
            raw_angles = angles.to_raw()
        elif isinstance(angles, list):
            raw_angles = O7Angle.from_list(angles).to_raw()
        else:
            raise ValidationError(
                f"Expected O7Angle or list, got {type(angles).__name__}"
            )
    ...
    def get_blocking(self, timeout_ms: float = 100) -> AngleData:
        ...
        This method sends a sensing request and blocks until 7 current angles
    ...
    def _on_message(self, msg: can.Message) -> None:
        ...
        # Validate angle count (should be 7 angles)
        if len(raw_angles) != self._ANGLE_COUNT:
            return

        angles = O7Angle.from_raw(raw_angles)

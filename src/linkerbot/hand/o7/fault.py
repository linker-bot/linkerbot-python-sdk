"""Fault management for O7 robotic hand.
...
class O7Fault:
    ...
    Attributes:
        ...
        thumb_rotation: Thumb rotation motor fault code
    ...
    ring: FaultCode
    pinky: FaultCode
    thumb_rotation: FaultCode

    def to_list(self) -> list[FaultCode]:
        """Convert to list of FaultCode in joint order.

        Returns:
            List of 7 joint fault codes [thumb_flex, thumb_abd, index, middle, ring, pinky, thumb_rotation]
        """
        return [
            self.thumb_flex,
            self.thumb_abd,
    ...
    @classmethod
    def from_list(cls, values: list[FaultCode]) -> "O7Fault":
        """Construct from list of FaultCode enum values.

        Args:
            values: List of 7 FaultCode values
    ...
    @classmethod
    def from_raw(cls, values: list[int]) -> "O7Fault":
...
class FaultData:
    ...
    Attributes:
        faults: O7Fault instance containing fault codes for all joints.
...
class FaultManager:
    ...
    def _on_message(self, msg: can.Message) -> None:
        ...
        if len(msg.data) < 8 or msg.data[0] != self._READ_FAULT_CMD:
            return

        raw_codes = list(msg.data[1:8])

        if len(raw_codes) != self._JOINT_COUNT:
            return

        faults = O7Fault.from_raw(raw_codes)

"""L20 robotic hand control interface.

This module provides the main L20 class for controlling the L20 robotic hand
...
class L20:
    """Main interface for L20 robotic hand control.

    This class provides a unified interface for controlling the L20 robotic hand,
    ...
    The L20 class should be used as a context manager to ensure proper resource
    cleanup:
    ...
    def __init__(
        ...
        """Initialize the L20 robotic hand interface.
    ...
    def get_snapshot(self) -> L20Snapshot:
        """Get all sensor data as a single snapshot (non-blocking).

        Returns:
            L20Snapshot with the latest cached data from all sensors.
    ...
    def start_polling(
        ...
        for source, interval in intervals.items():
            t = threading.Thread(
                ...
                daemon=True,
                name=f"L20-Polling-{source.value}",
            )
            t.start()
    ...
    def close(self) -> None:
        """Close the L20 interface and release all resources.
    ...
    def _ensure_open(self) -> None:
        ...
        if self._closed:
            raise StateError(
                "L20 interface is closed. Create a new instance or use context manager."
            )

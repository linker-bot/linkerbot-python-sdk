"""Per-slot motion-time control for O30i (10 ms per protocol tick)."""
...
O30I_MOTION_TIME_TICK_MS = 10


@dataclass(frozen=True, slots=True)
class O30iMotionTimeData:
    ticks: tuple[int, ...]
    timestamp: float
    ...
    def milliseconds(self) -> tuple[int, ...]:
        ...
        return tuple(value * O30I_MOTION_TIME_TICK_MS for value in self.ticks)
...
class MotionTimeManager:
    ...
    def __init__(self, client: HandProtocolV1) -> None:
        self._client = client
        self._relay = DataRelay[O30iMotionTimeData]()
    ...
    def set_ticks(
        ...
        *,
        ...
        write_vector(
            self._client,
            main_index=protocol.O30I_MI_MOTION_TIME,
            values=normalized,
    ...
    def set_milliseconds(
        ...
        *,
        ...
        if len(normalized) != protocol.O30I_CONTROLLED_JOINT_COUNT:
            raise ValidationError(
                "motion_time_ms must contain "
                f"{protocol.O30I_CONTROLLED_JOINT_COUNT} values"
            )
        ticks: list[int] = []
        for index, value in enumerate(normalized):
            protocol._validate_int(value, f"motion_time_ms[{index}]")
            if value < 0 or value > 2550 or value % O30I_MOTION_TIME_TICK_MS:
                raise ValidationError(
    ...
    def get_blocking(self, timeout_ms: float = 100) -> O30iMotionTimeData:
    ...
    def get_snapshot(self) -> O30iMotionTimeData | None:
        return self._relay.snapshot()

    def _send_sense_request(self) -> None:
        self.get_blocking(timeout_ms=100)

    def _set_event_sink(self, sink: Callable[[O30iMotionTimeData], None]) -> None:

"""Speed control and sensing for O30i."""
...
class SpeedManager:
    """Read and write the measured 36-byte speed object ``0x02``."""

    def __init__(self, client: HandProtocolV1) -> None:
        self._client = client
        self._relay = DataRelay[O30iSpeedData]()
    ...
    def set_speeds(
        ...
        *,
        ...
        normalized = validate_u8_values(values, name="speeds")
        write_vector(
            self._client,
            main_index=protocol.O30I_MI_SPEED,
            values=normalized,
    ...
    def get_blocking(self, timeout_ms: float = 100) -> O30iSpeedData:
        data = O30iSpeedData(
            speeds=read_vector(
                self._client,
                main_index=protocol.O30I_MI_SPEED,
    ...
    def get_snapshot(self) -> O30iSpeedData | None:
        return self._relay.snapshot()

    def _send_sense_request(self) -> None:
        self.get_blocking(timeout_ms=100)

    def _set_event_sink(self, sink: Callable[[O30iSpeedData], None]) -> None:

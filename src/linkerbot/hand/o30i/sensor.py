"""O30i sensor-capability metadata from HOP object ``0x31``."""
...
class O30iSensorInfo:
    """Measured sensor descriptor; current O30i units may report NO_SENSOR."""
...
class SensorManager:
    ...
    def get_info(self, *, timeout_ms: float = 1000) -> O30iSensorInfo:
        payload = self._client.read(
            main_index=protocol.O30I_MI_SENSOR_INFO,
            length=protocol.O30I_SENSOR_INFO_BYTE_LENGTH,
            timeout_ms=timeout_ms,
        )
        return decode_sensor_info(payload)


def decode_sensor_info(payload: bytes) -> O30iSensorInfo:
    normalized = bytes(payload)
    if len(normalized) < protocol.O30I_SENSOR_INFO_BYTE_LENGTH:
        raise HandProtocolError(
            "O30i sensor info too short: expected "
            f"{protocol.O30I_SENSOR_INFO_BYTE_LENGTH} bytes, got {len(normalized)}"
        )
    data = normalized[: protocol.O30I_SENSOR_INFO_BYTE_LENGTH]

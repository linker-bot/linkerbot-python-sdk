"""Read-only product information for O30i object ``0x41``."""
...
class O30iDeviceInfo:
    """Decoded 225-byte O30i product-information object."""
...
class VersionManager:
    """Read complete O30i identity/version data without exposing writes."""

    def __init__(self, client: HandProtocolV1) -> None:
        self._client = client

    def get_device_info(self, *, timeout_ms: float = 1000) -> O30iDeviceInfo:
        payload = self._client.read(
            main_index=protocol.O30I_MI_PRODUCT_INFO,
            length=protocol.O30I_PRODUCT_INFO_BYTE_LENGTH,
            timeout_ms=timeout_ms,
        )
        return decode_device_info(payload)


def decode_device_info(payload: bytes) -> O30iDeviceInfo:
    """Decode the fixed offsets verified for product object ``0x41``."""
    normalized = bytes(payload)
    if len(normalized) < protocol.O30I_PRODUCT_INFO_BYTE_LENGTH:
        raise HandProtocolError(
            "O30i product info too short: expected "
            f"{protocol.O30I_PRODUCT_INFO_BYTE_LENGTH} bytes, got {len(normalized)}"
        )
    data = normalized[: protocol.O30I_PRODUCT_INFO_BYTE_LENGTH]
    return O30iDeviceInfo(

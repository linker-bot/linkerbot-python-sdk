"""Temperature-state sensing for O30i."""
...
class TemperatureManager:
    ...
    def __init__(self, client: HandProtocolV1) -> None:
        self._client = client
        self._relay = DataRelay[O30iTemperatureData]()

    def get_blocking(self, timeout_ms: float = 100) -> O30iTemperatureData:
        data = O30iTemperatureData(
            temperatures=read_vector(
                self._client,
                main_index=protocol.O30I_MI_TEMPERATURE,
    ...
    def get_snapshot(self) -> O30iTemperatureData | None:
        return self._relay.snapshot()

    def _send_sense_request(self) -> None:
        self.get_blocking(timeout_ms=100)

    def _set_event_sink(self, sink: Callable[[O30iTemperatureData], None]) -> None:

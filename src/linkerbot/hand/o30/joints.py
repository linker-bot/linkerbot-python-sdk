"""O30 physical-joint definitions and normalized angle values."""
...
O30_JOINT_COUNT = protocol.O30_CONTROLLED_JOINT_COUNT


@dataclass(frozen=True, slots=True)
class JointSpec:
    """One physical O30 joint and its public metadata.
    ...
    defined separately by :class:`O30Angle` and is inverse to the raw bytes.
    """
...
O30_JOINT_SPECS: tuple[JointSpec, ...] = (
    JointSpec(
...
O30_DEFAULT_RAW_VALUES: tuple[int, ...] = tuple(
    spec.default for spec in O30_JOINT_SPECS
)


@dataclass(frozen=True, slots=True)
class O30Angle:
    """Twenty physical O30 joint values represented as percentages.

    Hardware verification confirms that O30 raw positions run opposite to the
    ...
    @classmethod
    def from_list(cls, values: list[float] | tuple[float, ...]) -> O30Angle:
        """Construct from 20 numeric percentages in public joint order."""
        return cls(tuple(values))

    @classmethod
    def from_raw(cls, values: list[int] | tuple[int, ...]) -> O30Angle:
    ...
    @classmethod
    def defaults(cls) -> O30Angle:
        """Construct the device's documented default 20-joint pose."""
        return cls.from_raw(O30_DEFAULT_RAW_VALUES)
...
def validate_u8_values(
    ...
    *,
    ...
    if len(normalized) != O30_JOINT_COUNT:
        raise ValidationError(
            f"{name} must contain {O30_JOINT_COUNT} values, got {len(normalized)}"
        )
...
def _validate_percentage_values(values: tuple[float, ...]) -> None:
    if len(values) != O30_JOINT_COUNT:
        raise ValidationError(
            f"angles must contain {O30_JOINT_COUNT} values, got {len(values)}"
        )

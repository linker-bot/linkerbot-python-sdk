...
class O30:
    """O30 robotic hand using the measured HandProtocol_v1.0/HOP behavior.
    ...
    def __init__(
        self,
        *,
        request_id: int = protocol.O30_REQUEST_ID,
        response_id: int | None = None,
        device: int = 0,
        channel: int = 0,
        library_path: str | Path | None = None,
        config: CANFDConfigOptions | None = None,
        frame_type: int | None = protocol.O30_FRAME_TYPE,
        ...
        """Open one O30 endpoint and construct all safe managers.

        Args:
            request_id: Standard CAN ID used for HOP requests. Defaults to the
                measured O30 endpoint ``0x001``.
            ...
            dispatcher: Existing dispatcher for tests, custom transports, or
                shared buses. When supplied, the O30 does not own or stop it.
            ...
            data_bitrate: SocketCAN data-phase configuration in bits per
                second. O30 frames do not enable BRS, but the Linux FD link
    ...
    def get_snapshot(self) -> O30Snapshot:
        ...
        try:
            for source, interval in selected.items():
                thread = threading.Thread(
                    ...
                    daemon=True,
                    name=f"O30-Polling-{source.value}",
                )
                thread.start()
    ...
    def _ensure_open(self) -> None:
        ...
        if self._closed:
            raise StateError("O30 interface is closed")

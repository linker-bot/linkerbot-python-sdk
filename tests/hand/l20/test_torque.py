"""Tests for L20 TorqueManager with hardware."""
...
from linkerbot.hand.l20 import AngleEvent, L20Torque, SensorSource
...
class TestTorqueManagerBlocking:
    ...
    def test_get_blocking_returns_valid_data(self, l20_hand: L20):
        ...
        data = l20_hand.torque.get_blocking(timeout_ms=500)
    ...
    def test_set_torques_with_list(self, l20_hand: L20):
        """set_torques should accept list[float] without error."""
        l20_hand.torque.set_torques([50.0] * 16)

    def test_set_torques_with_l20_torque(self, l20_hand: L20):
        """set_torques should accept L20Torque instance without error."""
        l20_hand.torque.set_torques(L20Torque.from_list([50.0] * 16))
...
class TestTorqueManagerSnapshot:
    ...
    def test_snapshot_populated_after_read(self, l20_hand: L20):
        ...
        l20_hand.torque.get_blocking(timeout_ms=500)
        data = l20_hand.torque.get_snapshot()
        assert data is not None
        assert len(data.torques) == 16
...
class TestTorqueInteractive:
    ...
    def test_torque_levels(
        ...
        def track(label: str, target: list[float]) -> None:
            elapsed, timed_out = _move_and_time(l20_hand, target)
        ...
        for level, torque_val in [("LOW", 10.0), ("MID", 50.0), ("HIGH", 100.0)]:
            ...
            session.step(
                ...
                action=lambda lbl=f"{level} close", tv=torque_val: (
                    l20_hand.torque.set_torques([tv] * 16),
                    track(lbl, CLOSED),
                ),
                expected=(

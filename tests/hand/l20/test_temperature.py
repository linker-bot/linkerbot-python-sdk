"""Tests for L20 TemperatureManager with hardware."""
...
class TestTemperatureManagerBlocking:
    ...
    def test_get_blocking_returns_valid_data(self, l20_hand: L20):
        ...
        data = l20_hand.temperature.get_blocking(timeout_ms=100)
    ...
    def test_temperature_field_access(self, l20_hand: L20):
...
class TestTemperatureManagerSnapshot:
    ...
    def test_snapshot_populated_after_read(self, l20_hand: L20):
        ...
        l20_hand.temperature.get_blocking(timeout_ms=100)

        data = l20_hand.temperature.get_snapshot()
...
class TestTemperatureExercise:
    ...
    def test_temperature_after_exercise(self, l20_hand: L20):
        """Temperatures should be readable after exercising the hand."""
        # Read baseline
        baseline = l20_hand.temperature.get_blocking(timeout_ms=100)
        assert baseline is not None
        baseline_list = baseline.temperatures.to_list()
        print(f"\n  Baseline temps: {[f'{t:.1f}' for t in baseline_list]}")

        # Exercise: open/close polling
        l20_hand.start_polling({SensorSource.TEMPERATURE: 0.1})
        time.sleep(1.0)
        l20_hand.stop_polling()

        # Read after exercise
        after = l20_hand.temperature.get_blocking(timeout_ms=100)

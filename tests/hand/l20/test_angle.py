"""Tests for L20 AngleManager with hardware."""
...
from linkerbot.hand.l20 import AngleEvent, L20Angle, SensorSource
from tests.conftest import InteractiveSession

pytestmark = [pytest.mark.l20, pytest.mark.control]
...
class TestAngleManagerBlocking:
    ...
    def test_get_blocking_returns_valid_data(self, l20_hand: L20):
        ...
        data = l20_hand.angle.get_blocking(timeout_ms=500)
    ...
    def test_get_blocking_has_timestamp(self, l20_hand: L20):
...
class TestAngleManagerSetAngles:
    ...
    def test_set_angles_with_list(self, l20_hand: L20):
        """set_angles should accept list[float] without error and allow read-back."""
        l20_hand.angle.set_angles(OPEN)
        time.sleep(2.0)
    ...
    def test_set_angles_with_l20_angle(self, l20_hand: L20):
        """set_angles should accept L20Angle instance without error and allow read-back."""
        target = L20Angle(
            thumb_abd=100.0,
            thumb_yaw=100.0,
            thumb_root1=100.0,
        ...
        l20_hand.angle.set_angles(target)
        time.sleep(2.0)
        ...
        assert len(data.angles) == 16
        print(
            f"\n  Read-back after set_angles (L20Angle): {[f'{a:.1f}' for a in data.angles.to_list()]}"
        )

    def test_set_all_closed(self, l20_hand: L20):
        """Set closed grip pose and verify read-back within tolerance."""
        _move_until_settled(l20_hand, CLOSED, timeout_sec=10.0)
    ...
    def test_set_all_open(self, l20_hand: L20):
        """Set open pose and verify read-back within tolerance."""
        _move_until_settled(l20_hand, OPEN)
...
class TestAngleManagerSnapshot:
    ...
    def test_snapshot_populated_after_blocking_read(self, l20_hand: L20):
        """After get_blocking(), get_snapshot() should return non-None with 16 angles."""
        l20_hand.angle.get_blocking(timeout_ms=500)

        data = l20_hand.angle.get_snapshot()
        assert data is not None
        assert len(data.angles) == 16

    def test_set_and_read_within_tolerance(self, l20_hand: L20):
...
class TestAngleInteractive:
    ...
    def test_individual_finger_movement(
        self, l20_hand: L20, interactive_session: InteractiveSession
    ):
        ...
        l20_hand.speed.set_speeds([100.0] * 16)
    ...
    def test_gradual_movement(
        ...
        for pct in [0, 25, 50, 75, 100]:
            ...
            session.step(
                ...
                action=lambda t=target: _move_until_settled(l20_hand, t),

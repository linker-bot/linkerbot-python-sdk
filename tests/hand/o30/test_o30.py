...
from linkerbot.hand.o30 import O30, AngleEvent, SensorSource, protocol
from tests.hand.o30.fakes import FakeO30Dispatcher

pytestmark = [pytest.mark.o30, pytest.mark.canfd]


def test_o30_is_lazily_exported_at_package_and_top_level() -> None:
    from linkerbot.hand import O30 as HandO30

    assert linkerbot.O30 is O30
    assert HandO30 is O30


def test_o30_defaults_match_measured_standard_can_endpoint() -> None:
    dispatcher = FakeO30Dispatcher()

    with O30(dispatcher=dispatcher) as hand:
...
def test_o30_supports_custom_standard_request_response_ids() -> None:
    dispatcher = FakeO30Dispatcher(response_id=0x456)
...
def test_o30_validates_endpoints_before_opening_backend(
    ...
    import linkerbot.hand.o30.o30 as o30_module

    monkeypatch.setattr(o30_module, "CANFDMessageDispatcher", fail_if_opened)
    with pytest.raises(ValidationError, match=message):
        O30(**kwargs)  # ty: ignore[invalid-argument-type]
    assert opened is False


def test_socketcan_requires_channel() -> None:
    with pytest.raises(ValidationError, match="socketcan_channel"):
        O30(interface_type="socketcan")
...
def test_socketcan_routes_all_link_configuration(
    ...
    class StubDispatcher(FakeO30Dispatcher):
        def __init__(self, **kwargs: object) -> None:
            super().__init__()
    ...
    monkeypatch.setattr(o30_module, "SocketCANFDBackend", StubBackend)
    monkeypatch.setattr(o30_module, "CANFDMessageDispatcher", StubDispatcher)
...
def test_ctypes_routes_vendor_adapter_configuration(
    ...
    with O30(device=2, channel=3, library_path="/tmp/libcanbus.so"):
        pass
...
def test_external_dispatcher_is_unsubscribed_but_not_stopped() -> None:
    dispatcher = FakeO30Dispatcher()
    hand = O30(dispatcher=dispatcher)
...
def test_owned_dispatcher_is_stopped_if_protocol_construction_fails(
    ...
    created: list[FakeO30Dispatcher] = []
    ...
    monkeypatch.setattr(o30_module, "CANFDMessageDispatcher", StubDispatcher)
    monkeypatch.setattr(o30_module, "HandProtocolV1", fail_protocol)
...
def test_snapshot_and_stream_receive_blocking_angle_read() -> None:
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_POSITION: bytes(range(36))})
...
def test_polling_validation_and_partial_start_rollback(
    ...
    dispatcher = FakeO30Dispatcher(
        {
            protocol.O30_MI_POSITION: bytes(36),
            protocol.O30_MI_SPEED: bytes(36),
        }
    )
    with O30(dispatcher=dispatcher) as hand:
...
def test_dunder_del_swallows_close_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    hand = O30(dispatcher=FakeO30Dispatcher())

...
from linkerbot.hand.o30.sensor import decode_sensor_info
from linkerbot.hand.o30.version import decode_device_info
...
def _product_info() -> bytes:
    data = bytearray(protocol.O30_PRODUCT_INFO_BYTE_LENGTH)
    fields = (
        (0x00, 0x10, "O30"),
        (0x10, 0x10, "12V~24V"),
        (0x20, 0x19, "MCU-UID-123"),
        (0x39, 0x20, "DEVICE-UID-O30"),
        (0x59, 0x08, "HOP"),
        (0x61, 0x08, "0.0.3"),
        (0x69, 0x08, "V1.2.0"),
        (0x71, 0x08, "V1.1.0"),
...
def test_product_info_reads_and_reassembles_all_225_bytes() -> None:
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_PRODUCT_INFO: _product_info()})

    with O30(dispatcher=dispatcher) as hand:
        info = hand.version.get_device_info()

    assert info.product_model == "O30"
...
def test_sensor_info_reports_no_sensor_without_reading_data_channels() -> None:
    ...
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_SENSOR_INFO: bytes(payload)})
...
def test_diagnostics_reads_4f_as_normal_object() -> None:
    payload = bytes(range(protocol.O30_COMMUNICATION_ERROR_BYTE_LENGTH))
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_COMMUNICATION_ERROR: payload})

...
from linkerbot.hand.o30 import O30, protocol
...
@pytest.mark.parametrize(
    ...
    [
        ("speed", protocol.O30_MI_SPEED, "speeds"),
        ("acceleration", protocol.O30_MI_ACCELERATION, "accelerations"),
        ("current", protocol.O30_MI_CURRENT, "currents"),
        ("voltage", protocol.O30_MI_VOLTAGE, "voltages"),
        ("torque", protocol.O30_MI_TORQUE, "torques"),
        ("temperature", protocol.O30_MI_TEMPERATURE, "temperatures"),
        ("motion_time", protocol.O30_MI_MOTION_TIME, "ticks"),
        ("fault", protocol.O30_MI_FAULT, "faults"),
    ],
)
def test_runtime_managers_read_full_object_but_return_only_physical_joints(
    ...
    dispatcher = FakeO30Dispatcher({main_index: _RUNTIME})
...
def test_angle_reads_actual_and_target_vectors() -> None:
    ...
    dispatcher = FakeO30Dispatcher(
        {
            (protocol.O30_MI_POSITION, False): actual,
            (protocol.O30_MI_POSITION, True): target,
        }
    )
    ...
    slots = protocol.O30_JOINT_SLOT_INDICES
...
def test_angle_percentage_write_reverses_o30_raw_direction() -> None:
    dispatcher = FakeO30Dispatcher()
...
def test_motion_time_exact_millisecond_conversion() -> None:
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_MOTION_TIME: _RUNTIME})
...
def test_full_i16_read_is_reassembled_and_decoded() -> None:
    ...
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_POSITION_I16: payload})
...
def test_fault_data_preserves_raw_bytes_without_inventing_bit_meanings() -> None:
    ...
    wire_faults[5] = 3
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_FAULT: bytes(wire_faults)})
...
def test_fault_data_rejects_invalid_physical_joint_index(
    ...
    dispatcher = FakeO30Dispatcher({protocol.O30_MI_FAULT: bytes(36)})

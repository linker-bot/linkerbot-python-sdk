...
`HandProtocolV1` 是与具体手型、`libcanbus.so` 和 SocketCAN 解耦的对象协议层。O30 在它之上提供设备对象定义和安全的高层 manager。

## 帧格式

O30 默认使用标准帧请求 ID `0x001` 和响应 ID `0x401`：
...
每个 `O30` 实例通过 `hand.protocol` 暴露传输无关客户端：

```python
from linkerbot import O30

with O30(interface_type="socketcan", socketcan_channel="can0") as hand:
    # 读取实际值：CTRL 的 RTS=0

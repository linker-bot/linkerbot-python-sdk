...
L6 灵巧手配备 5 个手指的力传感器（拇指、食指、中指、无名指、小指），支持阻塞读取和流式读取两种模式。
...
持续接收传感器数据。参数：`interval_ms`（轮询间隔，毫秒）和 `maxsize`（队列大小）。

```python
queue = hand.force_sensor.stream(interval_ms=100)
try:
    for data in queue:

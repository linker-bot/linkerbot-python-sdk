...
通过 `hand.temperature` 读取 L20 灵巧手 16 个关节电机的温度数据（单位：°C）。
...
- `TemperatureData`: 包含 `temperatures`（L20Temperature）和 `timestamp`
...
```python
from linkerbot.hand.l20 import SensorSource, TemperatureEvent

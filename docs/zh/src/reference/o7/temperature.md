...
通过 `hand.temperature` 读取 O7 灵巧手 7 个关节电机的温度数据（单位：°C）。
...
```python
from linkerbot.hand.o7 import SensorSource, TemperatureEvent
...
with O7(side="left", interface_name="can0") as hand:
    ...
    print(f"拇指旋转：{data.temperatures.thumb_rotation}°C")

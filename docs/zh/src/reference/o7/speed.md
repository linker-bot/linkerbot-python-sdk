...
通过 `hand.speed` 控制和读取 O7 灵巧手各关节电机的运动速度。
...
from linkerbot.hand.o7 import O7Speed

# 使用列表
hand.speed.set_speeds([50.0, 50.0, 50.0, 50.0, 50.0, 50.0, 50.0])
...
```python
from linkerbot.hand.o7 import SensorSource, SpeedEvent
...
print(rpm_values)  # [93.33, 93.33, 93.33, 93.33, 93.33, 93.33, 93.33]
```
...
# 拇指慢速，其他手指快速
speed = O7Speed.from_rpm([60.0, 60.0, 120.0, 120.0, 120.0, 120.0, 60.0])
hand.speed.set_speeds(speed)
```
...
with O7(side="left", interface_name="can0") as hand:
    # 使用 RPM 设置速度
    speed = O7Speed.from_rpm([90.0, 90.0, 120.0, 120.0, 120.0, 120.0, 90.0])
    hand.speed.set_speeds(speed)

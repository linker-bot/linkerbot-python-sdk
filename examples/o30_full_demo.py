...
O30 高层 SDK 没有可靠的使能/失能接口。运行 ``--move`` 前必须确保设备
...
from linkerbot.hand.o30 import (
    O30,
...
def run() -> None:
    ...
    print(
        f"[O30] interface={args.interface}, request_id=0x{args.request_id:03X}, "
        f"response_id={_format_response_id(args.response_id)}"
    )
    hand = _open_hand(args)
    with hand:
        print(
            f"[O30] 已连接：request_id=0x{hand.request_id:03X}, "
        ...
        if args.move:
            _step(
                f"低幅度点动 {O30_JOINT_SPECS[joint_index].id}",
            ...
            print("\n[O30] 未传 --move：没有发送任何运动命令")

        _show_snapshot_summary(hand)

    print(f"\n[O30] 连接已关闭：closed={hand.is_closed()}")


def _open_hand(args: argparse.Namespace) -> O30:
...
def _show_joint_definitions() -> None:
    print("\n[O30] 20 个实际关节（SDK index -> wire SI）")
    ...
    print("  default_raw:", list(O30_DEFAULT_RAW_VALUES))


def _show_device_info(hand: O30, timeout_ms: float) -> None:
...
def _show_angles(hand: O30, timeout_ms: float) -> None:
...
def _show_faults(hand: O30, timeout_ms: float) -> None:
    faults = hand.fault.get_blocking(timeout_ms)
    print("  raw:", list(faults.faults))
    active = [
        O30_JOINT_SPECS[index].id
        for index in range(O30_JOINT_COUNT)
        if faults.has_fault(index)
    ]
    print("  non-zero joints:", active or "none")


def _show_sensor_info(hand: O30, timeout_ms: float) -> None:
...
def _show_diagnostics(hand: O30, timeout_ms: float) -> None:
...
def _poll_all_sources(hand: O30, duration_s: float) -> None:
    ...
    consumer = threading.Thread(target=consume, daemon=True, name="O30-Demo-Stream")
    consumer.start()
    intervals = {
...
def _move_one_joint(
    ...
    *,
    ...
    spec = O30_JOINT_SPECS[joint_index]
...
def _show_snapshot_summary(hand: O30) -> None:
    snapshot = hand.get_snapshot()
    print("\n[O30] manager snapshots")
    for name in (
        "angle",
        "speed",
        "acceleration",
...
def _print_joint_values(label: str, values: Sequence[int]) -> None:
    print(f"  {label}:")
    for index, (spec, value) in enumerate(zip(O30_JOINT_SPECS, values, strict=True)):
...
def _resolve_joint(value: str) -> int:
    ...
    if index < 0 or index >= O30_JOINT_COUNT:
        raise SystemExit(f"--joint index must be between 0 and {O30_JOINT_COUNT - 1}")
    return index
...
if __name__ == "__main__":
    try:
        ...
        print("\n[O30] 用户中断，正在释放连接", file=sys.stderr)
        sys.exit(130)
    except LinkerbotError as error:
        print(f"[O30] SDK 错误：{error}", file=sys.stderr)
        sys.exit(1)
